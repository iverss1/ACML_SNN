...
training results with some of the latest netes and training techniques. It favours canonical PyTorch
...
#                     default='F:/backup/Python_Study/deep-learning-for-image-processing-master/pytorch_classification/SNN_CV/data/cifar-10-batches-py',
...
# Model parameters
parser.add_argument('--model', default='sequencer2d_s', type=str, metavar='MODEL',
...
parser.add_argument('--input-size', default=(3,32,32), nargs=3, type=int,
                    metavar='N N N',
...
parser.add_argument('-b', '--batch-size', type=int, default=1000, metavar='N',
                    help='input batch size for training (default: 128)')
parser.add_argument('-vb', '--validation-batch-size', type=int, default=None, metavar='N',
                    help='validation batch size override (default: None)')
parser.add_argument('--load-model', default='/home/yj/yyp/SNN_CV/output/cifar_10_not_best_model_best.pth.tar', type=str, metavar='PATH',
...
parser.add_argument('--clip-grad', type=float, default=20, metavar='NORM',
...
def main():
    ...
    #torch.cuda.set_device('cuda:'+str("1,2"))
    if args.distributed:
    ...
    #torchinfo.summary(model, input_size=(args.batch_size, 3, 32, 32))
    ...
    dataset_train = create_dataset(
        args.dataset, root='./data', split=args.train_split, is_training=True,
        class_map=args.class_map,
        download=args.dataset_download,
        batch_size=args.batch_size,
        repeats=args.epoch_repeats)
    dataset_eval = create_dataset(
        args.dataset, root='./data', split=args.val_split, is_training=False,
        class_map=args.class_map,
        download=args.dataset_download,
        batch_size=args.batch_size)
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    #summary(model, input_size=(10000, 3, 32, 32))
    #summary(model(),  torch.zeros((10000, 3, 32, 32)))
    end = time.time()
...
# Compute MAC operations for each layer
# Compute MAC operations for each layer
# Compute MAC operations for each layer
# Compute MAC operations for each layer

# Print total number of MAC operations for the network
