...
#  The optimize result on Cifar-100 without spike activation
    ...
    def forward(self, x):
        ...
        if self.with_group:
            ...
            if indicator % 2 == 0:
                ...
                mem_temp_g2 = mem_tempg[:, 0:window_size, W - window_size:W, :].reshape(B, window_size * window_size,
                                                                                        -1)
                mem_temp_g3 = mem_tempg[:, H - window_size:H, 0:window_size, :].reshape(B, window_size * window_size,
                                                                                        -1)
                mem_temp_g4 = mem_tempg[:, H - window_size:H, W - window_size:W, :].reshape(B,
                                                                                            window_size * window_size,
                                                                                            -1)

                x=x.reshape(B, H * W,-1)
                mem_tempg=mem_tempg.reshape(B, H * W,-1)
                g1, mem_tempg, self.c2_mem = self.snn_g1(x, mem_tempg, self.c2_mem)
                ...
                g1, mem_tempg = g1.reshape(B, H, W, -1), mem_tempg.reshape(B, H, W, -1)
                ...
                if self.index == 3:
                    ...
                    patch_group_2 = x[:, 0:window_size, W - window_size:W, :].reshape(B, window_size * window_size, -1)
                    patch_group_3 = x[:, H - window_size:H, 0:window_size, :].reshape(B, window_size * window_size, -1)
                    patch_group_4 = x[:, H - window_size:H, W - window_size:W, :].reshape(B, window_size * window_size,
                                                                                          -1)
                    # print(patch_group_4.shape,"patch_group_4")
                    mem_temp_g1 = mem_tempg[:, 0:window_size, 0:window_size, :].reshape(B, window_size * window_size,
                                                                                        -1)
                    mem_temp_g2 = mem_tempg[:, 0:window_size, W - window_size:W, :].reshape(B,
                                                                                            window_size * window_size,
                                                                                            -1)
                    mem_temp_g3 = mem_tempg[:, H - window_size:H, 0:window_size, :].reshape(B,
                                                                                            window_size * window_size,
                                                                                            -1)
                    mem_temp_g4 = mem_tempg[:, H - window_size:H, W - window_size:W, :].reshape(B,
                                                                                                window_size * window_size,
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # padding
        # �������feature map��H��W����2������������Ҫ����padding
        # ȷʵ����Ҫpadding�ģ�ǰ�汻4������һ��������2����
        H = x.shape[1]
        W = x.shape[2]
        ...
        if pad_input:
            ...
            # ע�������Tensorͨ����[B, H, W, C]�����Ի�͹ٷ��ĵ���Щ��ͬ
            x = F.pad(x, (0, 0, 0, W % 2, 0, H % 2))

        # ����ط�����˼�� ��x0,Bά�Ȳ�����,������0Ϊ���,��2Ϊ�������ȡֵ,Cά�Ȳ�����
