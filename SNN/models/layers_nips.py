...
class SNNBase(nn.Module):
    def __init__(self,index,input_size: int, hidden_size: int,
        ...
        if with_fc:
            if union == "cat":
                if index == 0:                    
                    self.fc = nn.Linear(self.output_size, hidden_size)
    ...
    def forward(self, x):
        B, H, W, C = x.shape
        global indicator
        self.c1_mem = torch.zeros(x.shape[0], self.hidden_size, 1,1).cuda()
        self.c2_mem = torch.zeros(x.shape[0], self.hidden_size, 1,1).cuda()
        ...
        if self.with_group :
            ...
            if self.index <= 3:       
                ...
                if indicator%2==0: 
                    ...
                    patch_group_1 = x[:, 0:window_size, 0:window_size, :].transpose(1,3)
                    patch_group_2 = x[:, 0:window_size, W-window_size:W, :].transpose(1,3)
                    patch_group_3 = x[:, H-window_size:H, 0:window_size, :].transpose(1,3)
                    patch_group_4 = x[:, H-window_size:H, W-window_size:W, :].transpose(1,3)
                    g1,self.c1_mem,self.c2_mem = self.snn_g1(patch_group_1,self.c1_mem,self.c2_mem)
                    #print(self.c1_mem,"self.c1_mem1")
                    g2,self.c1_mem,self.c2_mem = self.snn_g1(patch_group_2,self.c1_mem,2*torch.sin((self.c1_mem*math.pi/4)*torch.cos((self.c2_mem*math.pi/4))))
                    #print(self.c1_mem,"self.c1_mem2")
                    g3,self.c1_mem,self.c2_mem = self.snn_g1(patch_group_3,self.c1_mem,2*torch.sin((self.c1_mem*math.pi/4)*torch.cos((self.c2_mem*math.pi/4))))
                    #print(self.c1_mem,"self.c1_mem3")
                    g4,self.c1_mem,self.c2_mem = self.snn_g1(patch_group_4,self.c1_mem,2*torch.sin((self.c1_mem*math.pi/4)*torch.cos((self.c2_mem*math.pi/4))))
                    ...
                    if self.union == "cat":
                        ...
                        #print(x.shape,"x_cat")
                    if self.with_fc:      
                        #print(x.shape,"x_fc_in")         
                        x = self.fc(x)
                        #x=x
                        #print(x.shape,"x_fc_out")
                else:
                    if self.index == 3:   
                        B, H, W, C = x.shape
                        window_size = int(x.shape[1]/2)
...
class SCNN(nn.Module):
    ...
    def forward(self,x):
        ...
        for t in range(time_step):
            ...
            #print(self.bntt1(self.conv1(patch)),"self.bntt1(self.conv1(patch))")
            self.mem_conv1 = self.leak_mem * self.mem_conv1 + self.bntt1(self.conv1(patch))
            ...
            out_prev1 = out.clone()
            self.mem_conv2 = self.leak_mem * self.mem_conv2 + self.bntt1(self.conv2(out_prev1))
